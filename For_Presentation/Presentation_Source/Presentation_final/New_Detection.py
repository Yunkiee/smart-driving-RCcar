...
def detect_lanes_img(img): #1
	...
	print(height, width)
	vertices = np.array([[(width*(1/5),height),(width*(2/5)-100, height*(2/3)-100), (width*(3/5)+250, height*(2/3)-100), (width*(4/5)+200,height)]], dtype=np.int32) 
	...
	vertices2 = np.array([[(width*(1/5),height),(width*(2/5)-100, height*(2/3)-100), (width*(3/5)+250, height*(2/3)-100), (width*(4/5)+200,height)]], dtype=np.int32) #Start  ROI영역 크기
	...
	View_center = width/2
	if left_fit_line[2] == right_fit_line[2] : #One Line detection 이전 폭을 넣는다.
		center_circle2 = (left_fit_line[3])
		if left_fit_line[2] < width/2 : #Left line detection
			center_circle1 = left_fit_line[2] + (width/2-left_fit_line[2]-50)
			print("Left line detection")
		elif right_fit_line[2] > width/2 : #Right line detectin
			center_circle1 = right_fit_line[2] - (right_fit_line[2]-width/2-50)
	...
	while 1:
		if View_center < int(center_circle1) and int(center_circle1) - View_center > 70 and int(center_circle1) - View_center < 300: 
			View_center=center_circle1
			#View_center+=10
			print("Right", View_center ," -> ", center_circle1)
		elif View_center > int(center_circle1) and View_center - int(center_circle1) > 70 and View_center - int(center_circle1) < 300:
			View_center=center_circle1
			#View_center-=10
...
cap = cv2.VideoCapture('/home/capstone/Desktop/Driving/Input_data_mp4/real.MOV')
